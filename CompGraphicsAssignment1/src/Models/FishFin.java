...
public class FishFin extends Fish implements Displayable
{
	...
	public void display(GLAutoDrawable drawable)
	{
		...
		gl.glBegin(GL2.GL_LINE_STRIP);
			for (int i = 0; i <= 300; i++)
			{
				if (i <= 25 || i >= 125)
				{
					...
					gl.glVertex2d(getX() + Math.sin(theta) * (bodyRadius / xModifier), y + Math.cos(theta) * (bodyRadius / yModifier));
				}
				if(i == 26)
				{
					gl.glEnd();
		...
		gl.glBegin(GL2.GL_LINES);
			...
			gl.glVertex2d(x + Math.sin(theta) * (bodyRadius / xModifier - 0.01), y + Math.cos(theta) * (bodyRadius / yModifier));
			
			gl.glVertex2f(getX(), y + 0.01f);
			theta = (2.0f * Math.PI * 240 / 300);
			gl.glVertex2d(getX() + Math.sin(theta) * (bodyRadius / xModifier - 0.01), y + Math.cos(theta) * (bodyRadius / yModifier));
