	...
	public void display(GLAutoDrawable drawable)
	{
		if (!full) 
		{
			...
			for (int i = 0; i <= 300; i++) {
				...
				gl.glVertex2d(x + deltaX + Math.sin(theta) * (outerEyeRadius),
						y + deltaY + Math.cos(theta) * (outerEyeRadius));
			}
			gl.glEnd();
			gl.glBegin(GL2.GL_TRIANGLE_FAN);
			gl.glColor3f(0, 0, 0);
			...
			for (int i = 0; i <= 300; i++) {
				double theta = (2.0f * Math.PI * i / 300);
				gl.glVertex2d(x + deltaX + 0.01f + Math.sin(theta) * (pupilRadius),
						y + deltaY + 0.01f + Math.cos(theta) * (pupilRadius));
			}
			gl.glEnd();
			gl.glBegin(GL2.GL_TRIANGLE_FAN);
			gl.glColor3f(1, 1, 1);
			...
			for (int i = 0; i <= 300; i++) {
				double theta = (2.0f * Math.PI * i / 300);
				gl.glVertex2d(x + deltaX + 0.02f + Math.sin(theta) * (pupilShineRadius),
						y + deltaY + 0.02 + Math.cos(theta) * (pupilShineRadius));
			}
			gl.glEnd();
		}
